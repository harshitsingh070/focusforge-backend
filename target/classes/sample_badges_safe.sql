-- Safe badge insertion - only adds badges that don't already exist
-- First, let's see what badges you already have
SELECT name, category, criteria_type, threshold FROM badges ORDER BY category, threshold;

-- Delete this line and the one above after viewing existing badges
-- Then run the INSERT statements below

-- Points Badges (only insert if not exists)
